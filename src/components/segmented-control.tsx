...
export function SegmentedControl<TValue extends string>({
  ...
  ariaLabel: string;
  variant?: "micro" | "compact" | "floatingCompact" | "default";
  renderIcon?: (item: SegmentedControlItem<TValue>, isActive: boolean) => ReactNode;
}) {
  const isMicro = variant === "micro";
  const isCompact = variant === "compact";
  const isFloatingCompact = variant === "floatingCompact";
  const buttonClassName = `${isMicro || isFloatingCompact ? "" : "tap-target"} inline-flex ${
    isMicro ? "h-6 w-6" : isCompact ? "h-7 w-7" : isFloatingCompact ? "h-8 w-8" : "h-9 w-9"
  ...
  return (
    <div
      className={`inline-flex items-center rounded-full border border-[var(--theme-toggle-border)] bg-[var(--theme-toggle-surface)] ${
        isMicro ? "p-[0.1875rem] shadow-none" : isCompact ? "p-0.5 shadow-none" : isFloatingCompact ? "p-0.5 shadow-[var(--theme-toggle-shadow)] backdrop-blur" : "p-1 shadow-[var(--theme-toggle-shadow)] backdrop-blur"
      }`}
      role="group"
